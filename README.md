...
Depois, quando estiver na pasta do projeto, basta acessar o projeto frontend na pasta ``` web ``` digitar no cmd a seguinte instrução: **(dentro da pasta ``` web ``` do projeto clonado)**

```
> npm install
```

Depois, faz o mesmo processo na pasta ``` backend ```, basta acessar o pasta ``` backend ``` e digitar no cmd a seguinte instrução: **(dentro da pasta ``` backend ``` do projeto clonado)**
...
**(dentro da pasta ``` backend ``` do projeto clonado)** abra outro terminal para o projeto ser executado e digite:
```
yarn dev
```
...
Caso tenham dúvidas sobre o código do projeto, sintam-se a vontade em abrir uma **[ISSUE AQUI](https://github.com/Wanderson-A-Timoteo/happy-web/issues)**. Assim que possível, estarei respondendo as todas as dúvidas que tiverem!
...
* Projeto construído durante a Next Level Week #3 da [Rocketseat](https://rocketseat.com.br/). 

* Agradecimento ao [Diego Fernandes](https://github.com/maykbrito) por compartilhar conteúdo relevante e com qualidade de ensino.
